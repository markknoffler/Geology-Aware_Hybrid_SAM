...
- **Conv–BN–ReLU** \( \psi_{c_{out}}(\mathbf{x}) = \mathrm{ReLU}( \mathrm{BN}( \mathrm{Conv}_{7/4}(\mathbf{x})))\) for the stem, where \(\mathrm{Conv}_{7/4}\) denotes kernel \(7\), stride \(4\) (spatial downsample \(\times 4\)).
...
- **Down block** \(\mathcal{D}_{c \to 2c}(\mathbf{x}) = \mathrm{ReLU}(\mathrm{BN}(\mathrm{Conv}_{3/2}(\mathbf{x})))\) (stride 2).
...
\[
(\nabla_x H)_{i,j} = H_{i,j+1} - H_{i,j}, \quad (\nabla_y H)_{i+1,j} = H_{i+1,j} - H_{i,j}
\]
...
\[
\hat{\mathbf{L}}_{aux} = \mathrm{Conv}_{1\times1}(\mathrm{ReLU}(\mathrm{BN}(\mathrm{Conv}_{3\times3}(\mathbf{F}^{(0)}))))
\]
...
**FM loss**:
\[
\mathcal{L}_{fm} = \mathbb{E}_{t,\boldsymbol{\epsilon}}\left[ \|\mathbf{v}_\theta - (\boldsymbol{\epsilon}-\mathbf{z})\|_2^2 \right].
\]
...
\[
\mathcal{L}_{v} = \mathbb{E}\left[ \|\mathbf{v}_\theta(\mathbf{x}_t, t) - \mathbf{v}_\theta(\mathbf{x}_t, t')\|_2^2 \right].
\]
...
Readout logits from final latent: \(\hat{\mathbf{L}}_{flow} = \mathrm{Conv}_{1\times1}^{ro}(\mathbf{x})\).

**Combined output** (evaluation):
\[
\hat{\mathbf{L}} = \hat{\mathbf{L}}_{aux} + \alpha_{cfm} \, \hat{\mathbf{L}}_{flow}
\]
with \(\alpha_{cfm}\) = `--flow_combine_scale` (default 0.5).
...
\[
\mathcal{L} = \lambda_{seg}\mathcal{L}_{Tversky}(\hat{\mathbf{L}}_{aux}, \mathbf{Y})
