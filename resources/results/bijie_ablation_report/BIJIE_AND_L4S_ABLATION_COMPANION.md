...
Pixel F1/IoU in the summary CSVs do not directly answer whether the model **fires on scenes with no landslide**. That question is handled by a dedicated eval pass documented in depth here:

**`SAM/resources/results/LANDSLIDE_PRESENCE_DETECTION.md`**

| Item | Detail |
| --- | --- |
| Script | `SAM/model_architecture_cfm_landseg/eval/eval_landslide_presence.py` |
| Wrapper | `SAM/resources/results/run_landslide_presence_eval.sh` |
| Epochs | From summary CSV: Bijie **112**, L4S **111** for `tri_encoder_cfm_v2` |
| Checkpoints | `SAM/runs/bijie/tri_encoder_cfm_v2/checkpoints/epoch_XXXX.pt` (and L4S twin) |
| Bijie negatives | `non-landslide/image` with implicit empty GT mask on val split |
| L4S negatives | Empty-mask tiles in the 10% val index split only |
| Outputs | `landslide_presence_report/tri_encoder_presence_combined_table.csv` + score histogram PNG |

Run on the server after setting `BIJIE_ROOT` and `L4S_ROOT`; paste the filled combined table and figure into the manuscript (see section 5 of the presence doc).
